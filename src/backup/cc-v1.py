...
def build_model(final_simulation_time=None, event_logger=None, verbose=True,
    ...
    def make_time_dependent_arrival(base_dist, arrival_slots, env=model.env, num_sources=1, start_day_of_week=2):
        """
        Retorna uma função que gera tempos de inter-chegada escalados por hora E dia da semana.
        
        start_day_of_week: 0=Segunda, 1=Terça, 2=Quarta, ..., 6=Domingo.
        Como seu dashboard usa a data base 2025-01-01 (uma Quarta-feira), o padrão é 2.
        """
        DAYS_MINUTES = 1440
        num_slots = len(arrival_slots)

        # Fatores calibrados a partir da tabela fornecida (Média do Dia / Média Geral Semanal)
        # Garantem que a média total da simulação permaneça fiel à distribuição base.
        # Média Semanal Geral = (14.74 + 15.67 + 14.98 + 15.84 + 14.58 + 8.88 + 8.64)/7 
        # = 93.33/7 = 13.3328
        WEEKDAY_FACTORS = {
            0: 14.74 / 13.332857,  # Segunda-feira (1.1055)
            1: 15.67 / 13.332857,  # Terça-feira   (1.1753)
            2: 14.98 / 13.332857,  # Quarta-feira  (1.1235)
            3: 15.84 / 13.332857,  # Quinta-feira  (1.1880)
            4: 14.58 / 13.332857,  # Sexta-feira   (1.0935)
            5: 8.88 / 13.332857,   # Sábado        (0.6660) ← Queda no FDS
            6: 8.64 / 13.332857,   # Domingo       (0.6480) ← Queda no FDS
        }

        def _sample_base():
            """Extrai uma amostra da distribuição base."""
            return base_dist() if callable(base_dist) else float(base_dist)

        def time_dependent_arrival():
            # ── 1. Calcular o dia da semana atual na simulação ────────────────
            sim_day = int(env.now // DAYS_MINUTES)
            current_day_of_week = (start_day_of_week + sim_day) % 7
            day_factor = WEEKDAY_FACTORS[current_day_of_week]

            # ── 2. Localizar o slot de 2 horas atual do dia ───────────────────
            current_hour = (env.now % DAYS_MINUTES) / 60.0

            slot_fraction = arrival_slots[-1][2]  # Fallback para o último slot
            for start_h, end_h, fraction in arrival_slots:
                if start_h <= current_hour < end_h:
                    slot_fraction = fraction
                    break

            # ── 3. Calcular a intensidade relativa combinada (Hora x Dia) ─────
            k_hourly = slot_fraction * num_slots
            k_total = k_hourly * day_factor

            # ── 4. Amostrar e Escalar o tempo de inter-chegada ────────────────
            # k_total > 1 (Picos)    → Divisor maior → Inter-chegada menor (Mais pacientes)
            # k_total < 1 (FDS/Noite) → Divisor menor → Inter-chegada maior (Menos pacientes)
            return (_sample_base() / num_sources) / k_total
